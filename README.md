...
#### Why Use the Git Hook?

EDOG modifies files in your FLT codebase to inject tokens. If you accidentally commit these changes, it could:
- Break the build for other developers
- Expose token-related code in the repository
- Cause merge conflicts

The pre-commit hook **automatically blocks** any commit that contains EDOG modifications.
...
PS C:\flt> edog --install-hook
✅ Installed EDOG pre-commit hook
   Location: C:\flt\.git\hooks\pre-commit
   Commits with EDOG changes will now be blocked.

# Later, try to commit with EDOG changes still applied...
PS C:\flt> git add .
PS C:\flt> git commit -m "my feature"

COMMIT BLOCKED: EDOG DevMode changes detected!

   File: LiveTableController.cs contains EDOG modifications.
   Run 'edog --revert' before committing.

# Fix it by reverting EDOG changes first
PS C:\flt> edog --revert
✅ All EDOG changes reverted

PS C:\flt> git commit -m "my feature"
[main abc1234] my feature
```

<br/>

#### Files Monitored by Hook

The hook watches these EDOG-modified files:
- `LiveTableController.cs`
- `LiveTableSchedulerRunController.cs`
- `GTSOperationManager.cs`
