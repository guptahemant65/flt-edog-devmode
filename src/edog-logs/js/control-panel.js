...
class ControlPanel {
  ...
  async _fetchConfig() {
    ...
    if (!resp.ok) throw { status: resp.status, body: await resp.json() };
  ...
  async _cancelDag(iterationId) {
    const resp = await fetch('/api/flt/canceldag/' + iterationId, { method: 'POST' });
    if (!resp.ok) throw { status: resp.status, body: await resp.json() };
    return await resp.json();
  }
    ...
    this.connBar.innerHTML =
      ...
      (expired
        ? '<div class="cp-banner warning">Run <code>edog --refresh-token</code> in terminal ' +
          '<button class="btn-copy" onclick="navigator.clipboard.writeText(\'edog --refresh-token\')">copy</button></div>'
        : '');
  }
  ...
  _renderIdRow(label, value) {
    ...
    return '<div class="conn-id-row">' +
      '<span class="conn-id-label">' + label + '</span>' +
      '<code class="conn-id-value">' + value + '</code>' +
      '<button class="btn-copy" onclick="navigator.clipboard.writeText(\'' + value + '\')">copy</button>' +
    '</div>';
  }
  ...
  _renderConnectionError(err) {
    ...
    this.connBar.innerHTML =
      ...
      '</div>' +
      '<div class="cp-banner error">' + msg +
        ' <button class="btn-secondary" onclick="window.edogViewer.controlPanel.activate()">Retry</button>' +
      '</div>';
  }
  ...
  _renderDagOverview(dagData) {
    ...
    if (!nodes || nodes.length === 0) {
      this.dagOverview.innerHTML =
        '<div class="overview-header">' +
          '<h3>DAG Overview</h3>' +
          '<button class="btn-secondary" onclick="window.edogViewer.controlPanel._refreshDag()">&#8635; Refresh</button>' +
        '</div>' +
    ...
    var rowsHtml = nodes.map(function(n) {
      var deps = (n.dependencies && n.dependencies.length) ? n.dependencies.join(', ') : '\u2014';
      return '<tr>' +
        '<td class="node-name">' + n.name + '</td>' +
        '<td class="node-type">' + (n.type || '\u2014') + '</td>' +
    ...
    this.dagOverview.innerHTML =
      '<div class="overview-header">' +
        ...
        '<div class="overview-badges">' +
          ...
          (refreshMode ? '<span class="status-pill">' + refreshMode + '</span>' : '') +
        '</div>' +
        '<button class="btn-secondary" onclick="window.edogViewer.controlPanel._refreshDag()">&#8635; Refresh</button>' +
      '</div>' +
  ...
  _renderDagError(err) {
    ...
    if (status === 401 || status === 403 || body.error === 'token_expired') {
      ...
      banner = '<div class="cp-banner error">Cannot reach Fabric service. Check VPN/network. ' +
        '<button class="btn-secondary" onclick="window.edogViewer.controlPanel._refreshDag()">Retry</button></div>';
    } else {
      banner = '<div class="cp-banner error">Server error (' + status + '): ' + (body.message || 'Unknown') +
        ' <button class="btn-secondary" onclick="window.edogViewer.controlPanel._refreshDag()">Retry</button></div>';
    }
  ...
  _renderExecutionIdle() {
    ...
    if (lastExec) {
      ...
      lastSummary = '<div class="last-exec-summary">' +
        'Last: <span class="status-pill ' + statusClass + '">' + statusIcon + ' ' + st + '</span>' +
        (dur ? ' \u00b7 ' + dur : '') + (ago ? ' \u00b7 ' + ago : '') +
      '</div>';
    }
    ...
    this.execCard.innerHTML =
      '<button class="btn-run"' + (disabled ? ' disabled title="' + disabledReason + '"' : '') +
        ' onclick="window.edogViewer.controlPanel._handleRunDag()">' +
        '\u25b6 Run DAG' +
      '</button>' + lastSummary;
  }
  ...
  _renderExecutionActive(iterationId, execution) {
    ...
    var shortId = iterationId ? iterationId.substring(iterationId.length - 8) : '...';
    ...
    this.execCard.innerHTML =
      ...
      '<div class="exec-actions">' +
        '<button class="btn-cancel" onclick="window.edogViewer.controlPanel._handleCancelDag(\'' + iterationId + '\')">\u23f9 Cancel</button>' +
        '<button class="btn-secondary" onclick="window.edogViewer.controlPanel._handleWatchLogs()">\u2197 Watch Logs</button>' +
      '</div>';
  ...
  _renderHistory(executions) {
    ...
    var rows = executions.slice(0, 10).map(function(ex) {
      var shortId = ex.iterationId
        ? '...' + ex.iterationId.substring(ex.iterationId.length - 8)
        : '\u2014';
      var st = ex.status || '';
      ...
      return '<tr>' +
        ...
        '<td><span class="status-pill ' + statusClass + '">' + statusIcon + ' ' + st + '</span></td>' +
    ...
    this.historyEl.innerHTML =
      '<div class="history-header">' +
        ...
        '<button class="btn-secondary" onclick="window.edogViewer.controlPanel._fetchAndRenderHistory()">&#8635;</button>' +
      '</div>' +
