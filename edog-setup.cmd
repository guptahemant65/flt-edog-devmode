...
echo.
echo   %BCYAN%+----------------------------------------------+%RESET%
echo   %BCYAN%^|%RESET%  %BOLD%EDOG DevMode  %DIM%Setup%RESET%                        %BCYAN%^|%RESET%
...
echo   %BCYAN%[1/4]%RESET% %BOLD%Checking prerequisites%RESET%
echo.

python --version >nul 2>&1
if errorlevel 1 (
    echo     %BRED%x%RESET%  Python not found
    echo.
...
if errorlevel 1 (
    echo     %BRED%x%RESET%  .NET SDK not found
    echo.
...
echo   %BCYAN%[2/4]%RESET% %BOLD%Installing Python dependencies%RESET%

pip install rich --quiet --disable-pip-version-check >nul 2>&1
if errorlevel 1 (
    echo     %BYELLOW%!%RESET%  %YELLOW%rich%RESET% install failed - CLI will work without styling
    set /a ERRORS+=1
) else (
...
echo   %BCYAN%[3/4]%RESET% %BOLD%Building token-helper%RESET% %DIM%- Silent CBA auth%RESET%
...
if exist "%EDOG_DIR%\scripts\token-helper\bin\Debug\net8.0\token-helper.exe" (
    ...
    if errorlevel 1 (
        echo     %BRED%x%RESET%  Build failed
...
REM == Step 4: PATH ================================================
echo   %BCYAN%[4/4]%RESET% %BOLD%Adding edog to PATH%RESET%
...
if !ERRORS!==0 (
    echo   %BGREEN%+----------------------------------------------+%RESET%
    echo   %BGREEN%^|  Setup complete!%RESET%                               %BGREEN%^|%RESET%
    echo   %BGREEN%+----------------------------------------------+%RESET%
) else (
    echo   %BYELLOW%+----------------------------------------------+%RESET%
    echo   %BYELLOW%^|  Setup complete with !ERRORS! warning^(s^)%RESET%           %BYELLOW%^|%RESET%
    echo   %BYELLOW%+----------------------------------------------+%RESET%
)
echo.
